...
func (cg *connectorGenerator) genGetTypeValueDecoder(sb *connectorTypeBuilder, ty *TypeInfo, key string, fieldName string) {
	...
	case "int", "int8", "int16", "int32", "int64", "rune", "byte":
		if slices.Contains([]string{"[]int", "[]int8", "[]int16", "[]int32", "[]int64", "[]rune", "[]byte"}, strings.Join(ty.TypeFragments, "")) {
	...
	case "uint", "uint8", "uint16", "uint32", "uint64":
		if slices.Contains([]string{"[]uint", "[]uint8", "[]uint16", "[]uint32", "[]uint64"}, strings.Join(ty.TypeFragments, "")) {
	...
	case "float32", "float64":
		if slices.Contains([]string{"[]float32", "[]float64"}, strings.Join(ty.TypeFragments, "")) {
	...
	case "any", "interface{}":
		if slices.Contains([]string{"[]any", "[]interface{}"}, strings.Join(ty.TypeFragments, "")) {
